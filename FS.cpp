...
static bool sflags(oflag_t mode,  OpenMode& om, AccessMode& am);

File::File(FileImplPtr p , FS *baseFS ) : _p(p), _fakeDir(nullptr), _baseFS(baseFS) { 
     if (_baseFS) {
        _baseFS->_openFiles.insert(std::pair<File*,File*>(this,this));
     }
     _refcnt++;
}

File::File(const File &oldf) : _p(oldf._p), _fakeDir(oldf._fakeDir), _baseFS(oldf._baseFS) {
...
File::~File() {
   _refcnt--;
   if (_baseFS && _refcnt <= 0) {
      _baseFS->_openFiles.erase(this);
   }
 }
...
int File::available() {
    if (!_p)
        return false;
    DEBUGV("File::available() sz=%d pos=%d\n", _p->size(), _p->position());
    return _p->size() - _p->position();
}

int File::read() {
    if (!_p) {
        DEBUGV("File::read() no impl\n");
        return -1;
    }

    uint8_t result;
    int nr;
    if ((nr = _p->read(&result, 1)) != 1) {
        DEBUGV("FS::read() bad chars %d\n", nr);
        return -1;
    }
...
File Dir::openFile(const char* mode) {
    ...
    return openFile(om, am);
}

File Dir::openFile(oflag_t mode) {
    OpenMode om;
    AccessMode am;
    if (!sflags(mode, om, am)) {
        DEBUGV("Dir::openFile: invalid mode `%d`\r\n", mode);
        return File();
    }
    return openFile(om, am);
}

File Dir::openFile(OpenMode om, AccessMode am) {
    if (!_impl) {
        return File();
    }
...
File FS::open(const String& path, oflag_t mode) {
    return open(path.c_str(), mode);
}
File FS::open(const char* path, oflag_t mode) {

    OpenMode om;
    AccessMode am;
    if (!sflags(mode, om, am)) {
        DEBUGV("FS::open: invalid mode `%d`\r\n", mode);
        return File();
    }
    return open(path, om, am);
}
...
File FS::open(const char* path, OpenMode om, AccessMode am) {
    if (!_impl) {
        return File();
    }
...
static bool sflags(oflag_t mode, OpenMode& om, AccessMode& am) {
        om  =(OpenMode)0;
        if ( mode & O_CREAT) {
            om |=  (OpenMode)OM_CREATE;
        }
        if ( mode & O_AT_END) {
            om |= (OpenMode)OM_APPEND;
        }
        if ( mode & O_TRUNC) {
            om |= (OpenMode)OM_TRUNCATE;
       } 
        am =(AccessMode)0;
        if ( mode & O_READ) {
            am |=  (AccessMode)AM_READ;
        }
        if ( mode & O_WRITE) {
             am |= (AccessMode)AM_WRITE;
        }
        return true;
}
